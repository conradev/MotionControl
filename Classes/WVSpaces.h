...
@property(nonatomic) BOOL bringSpacesToCurrentSpace;
@property(nonatomic) BOOL preventMRUForNextCall;
@property(readonly, nonatomic) BOOL fluidSwitching;
@property(readonly, nonatomic) BOOL dashboardIsCurrent;
@property(readonly, nonatomic) BOOL anySwitchingOccuring;
@property(readonly, nonatomic) BOOL currentSpaceIsDashboard;

@property(readonly, nonatomic) unsigned long long firstUserSpaceIndex;
@property(readonly, nonatomic) unsigned long long currentSpaceIndex;

@property(readonly, nonatomic) NSArray *spaces;
@property(readonly, nonatomic) NSArray *userSpaces;
@property(readonly, nonatomic) NSArray *orderedUsersSpaces;

@property(readonly, nonatomic) WVSpace *mainSpace;
@property(readonly, nonatomic) WVSpace *dashboardSpace;
@property(readonly, nonatomic) WVSpace *previousSpace;
@property(readonly, nonatomic) WVSpace *nextSpace;
@property(readonly, nonatomic) WVSpace *currentSpace;

- (WVSpace *)spaceWithUUID:(NSString *)uuid;
- (WVSpace *)spaceAtIndex:(unsigned long long)index;
- (unsigned long long)indexOfSpace:(WVSpace *)space;
- (void)setSpaceIndexOutsideOfSpaces:(unsigned long long)index;

- (void)removeUserSpace:(WVSpace *)space;
- (WVSpace *)addUserSpace;
- (BOOL)canAddUserSpace;

- (void)moveSpace:(WVSpace *)space toIndex:(unsigned long long)index;
- (void)fluidGestureSwipe:(CGSEventRecord)record progress:(float)progress;
- (void)fluidGestureStop:(CGSEventRecord)record canceled:(BOOL)canceled velocity:(double)velocity forward:(BOOL)forward;
- (void)fluidGestureStart:(CGSEventRecord)record;

- (BOOL)switchToSpaceAtIndex:(unsigned long long)index fromServer:(BOOL)server updatePSN:(BOOL)update;
- (BOOL)switchToSpaceWithUUID:(NSString *)uuid;
- (BOOL)switchToUserSpace:(unsigned long long)index;
- (BOOL)switchToNextSpace:(BOOL)animated;
- (void)switchForWindowDrag:(BOOL)animated;
- (BOOL)switchToPreviousSpace:(BOOL)animated;
- (void)switchToLastSpace;

- (void)setBinding:(id)binding forBundleID:(NSString *)bundleID;
- (id)bindingForBundleID:(NSString *)bundleID;
- (void)mainDisplayScaleFactorChanged;
- (void)performDelayedSpaceChanges;
- (void)addDelayedSpaceChange:(id)notification;
- (void)spaceModifedWhenNotFront:(id)notification;
- (CGRect)rectForOffscreenWindow:(CGRect)windowRect space:(WVSpace *)space displaySize:(CGSize)displaySize;
- (void)appOrderedFrontWithNoWindows:(pid_t)pid;
- (BOOL)isPidFullscreen:(pid_t)pid;
- (void)applicationDied:(pid_t)pid;
- (void)dashboardPIDRemoved:(pid_t)pid;
- (void)dashboardPIDAdded:(pid_t)pid;
